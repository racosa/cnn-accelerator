...
class Convolution(object):
    ...
    def convolve(self, input):
        ...
        padded_input = np.zeros((input.shape[0] + 2*self.zero_padding, input.shape[1] + 2*self.zero_padding, self.input_depth))
        padded_input[self.zero_padding:input.shape[0]+self.zero_padding, self.zero_padding:input.shape[1]+self.zero_padding, :] = input 
    
        for z in range(self.number_of_kernels):
            for y in range(self.output_size):
                for x in range(self.output_size):
                    output[x][y][z] = np.sum(padded_input[x*self.stride : x*self.stride + self.kernel_size,
